...
public class Cipher2 {
	...
	public static void readFile(Record2[] alphabet) { // PART C
		Scanner input = null;
		String pathname = "Cipher2.txt";
	...
	public static void sortDescending(Record2[] alphabet) { // PART D
	...
	public static void sortOriginalLetter(Record2[] alphabet) { // PART G
		
	}

	public static void setNewLetters(Record2[] alphabet) {
		Scanner input = null;
		String pathname = "Replace2.txt";
		File file = new File(pathname);
		try {
			input = new Scanner(file);
		} catch (FileNotFoundException ex) {
	...
	public static void decode(Record2[] alphabet) { // PART E
		Scanner input = null;
		String pathname = "Cipher2.txt";
		File file = new File(pathname);
		try {
	...
	public static void writeNewFile(char[] dup1, char[] dup2, Record2 alphabet[]) { // PART F
		String pathname = "Uncoded2.txt";
		File file = new File(pathname);
		...
		Arrays.sort(alphabet, new OriginalCompare());
		for(int i=0; i<alphabet.length; i++) {
			System.out.println(alphabet[i].getLetter() + " " + alphabet[i].getFrequency());
		}
		Scanner input = null;
		String path2 = "Cipher2.txt";
		...
		System.out.println("\n\nUNCODED TEXT REMOVED FOR DUPLICATES IS NOW IN NEW FILE CALLED 'Uncoded2.txt' ");
		while (input.hasNextLine()) {
			output.println("\n"); // write to file
			String line = input.nextLine();
			char[] lineArray = line.toCharArray();
			...
			for (int j = 0; j < lineArray.length; j++) {
				int ascii = (int) lineArray[j]; 
				char newLet = ' '; 
				int adjusted = ascii-65; // because A starts at 65
				if(adjusted<26 && adjusted>0) { // if in range of letters
					newLet = alphabet[adjusted].getNewLetter();
					for(int k=0; k<dup1.length; k++) { // if 1 of the doubles to switch
						if(newLet == dup1[k]) { // if a duplicate
							newLet = dup2[k]; // switch with match
							lineArray[j] = newLet;
						}
						else if(newLet == dup2[k]) { // if a duplicate
